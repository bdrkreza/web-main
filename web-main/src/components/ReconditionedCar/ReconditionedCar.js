...
const useStyles = makeStyles((theme) => ({
  ...
  featuredSection: {
    ...
    boxShadow: "0px 3px 2.5px rgba(0,0,0,0.16)",
  },
  featuredContainer: {
    paddingTop: "58px",
    paddingBottom: "58px",
...
function ReconditionedCar(props) {
  ...
  useEffect(() => {
    try {
      (async () => {
        const {
          data: { results },
        } = await api.get(`api/cars/search-featured-car/?car_type=reconditioned`);
  ...
  return (
    <div className={classes.featuredSection} style={{ background: `${props.background}` }}>
      <Container maxWidth="lg" className={classes.featuredContainer}>
        <SectionTitle
          ...
          title2={langVariables !== null ? langVariables["reconditioned_car"].lang_content : "Reconditioned Cars"}
        />
        ...
        >
          {cars.map((car, i) => {
            return (
              <SwiperSlide key={i}>
                <FeaturedNewCarCard key={i} car={car} pathName={"featured-reconditioned-car"} />
              </SwiperSlide>
            );
          })}
          {/* {
...
export default ReconditionedCar;
