  ...
  useEffect(() => {
    try {
      (async () => {
        const {
          data: { results },
        } = await api.get(`api/cars/choose-by-maker/?maker_name=hyundai`);
  ...
  return (
    <div maxWidth="lg" className={classes.featuredContainer}>
      ...
      >
        {cars.map((car, i) => i < 5 && (
           <SwiperSlide key={i}>
             <PopularBrandCard key={i} car={car} pathName={'popular-hyundai'}/>
           </SwiperSlide>
        )
        )}
      </Swiper>
